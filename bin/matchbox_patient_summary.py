...
def get_args():
    ...
    parser.add_argument('-t', '--tumor', metavar='<tumor_type>', 
            help='Retrieve data for only this tumor type or comma separate list '
            'of tumors. Note that you must quote tumors with names containing '
            'spaces.')
...
def patient_summary(data,outfh,patients=None,outside=False,):
    ...
    if results:
        ...
        outfh.write(":::  MATCH {} Summary as of {} (Query PSNs: {}; Total "
            "Passed Biopsies: {})  :::\n".format(
                'Patient', today, len(results), num_collected_biopsies))

        outfh.write('PSN,BSN,MSN,Disease\n')
        for res in sorted(results.keys()):
            # just take last biopsy; don't need all.
...
if __name__=='__main__':
    ...
    if not args.json:
        sys.stdout.write('WARN: No MATCHBox JSON dump obj loaded. Performing '
        'live queries can take a few minutes, and be sped up by loading a JSON '
        'obj from `matchbox_json_dump.py` first.\n')
        sys.stdout.flush()
